  ...
  namespace recommender
  {
        ...
        httplib::Response &res)
    {
      try
      {
        ...
        for (const auto &[item_id, score] : recommendations)
        {
          response["recommendations"].push_back({{"item_id", item_id},
                                                 {"score", score}});
        }
        ...
        httplib::Response &res)
    {
      try
      {
        ...
        for (const auto &[item_id, score] : similar_items)
        {
          response["similar_items"].push_back({{"item_id", item_id},
                                               {"score", score}});
        }
    ...
    RecommenderService::RecommenderService(
        ...
        : BaseService(grpc_address, rest_address), recommender_(std::make_shared<LlamaBloomRecommender>(model_path))
    {
